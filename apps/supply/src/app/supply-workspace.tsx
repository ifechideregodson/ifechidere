...
export default function SupplyWorkspace() {
  ...
  async function buy(productId: string) { const response = await fetch(`${apiUrl}/v1/store-orders`, { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify({ productId, quantity: 1 }) }); if (!response.ok) { setMessage("Connect your account or check product availability."); return; } const order = await response.json(); setOrderId(order.id); const checkout = await fetch(`${apiUrl}/v1/orders/${order.id}/checkout`, { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify({ successUrl: `${window.location.origin}/?payment=success`, cancelUrl: `${window.location.origin}/?payment=cancelled` }) }); const data = await checkout.json().catch(() => null); if (checkout.ok && data?.url) window.location.assign(data.url); else setMessage(data?.error ?? "Payment checkout is not configured."); void loadProducts(); }
