...
function PulseWorkspace() {
  ...
  async function createPortfolio(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const response = await fetch(`${apiUrl}/v1/portfolios`, { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify({ title: portfolioTitle, slug: portfolioSlug, bio: portfolioBio }) }); setMessage(response.ok ? "Portfolio created." : "The portfolio could not be created."); }
